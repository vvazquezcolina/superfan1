...
export default async function Image({
  ...
  const title = isEs ? 'HERRAMIENTAS\nGRATUITAS' : 'FREE\nTOOLS'
  const subtitle = isEs ? 'Planifica tu aventura mundialista' : 'Plan your World Cup adventure'

  const tools = [
    { icon: '💱', label: isEs ? 'Moneda' : 'Currency' },
    { icon: '📊', label: isEs ? 'Presupuesto' : 'Budget' },
    { icon: '🧳', label: isEs ? 'Equipaje' : 'Packing' },
    { icon: '📅', label: isEs ? 'Itinerario' : 'Itinerary' },
    { icon: '🗺️', label: isEs ? 'Mapa' : 'Map' },
  ]
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              🛠️ {isEs ? '5 HERRAMIENTAS GRATUITAS' : '5 FREE TOOLS'}
            </div>
          </div>
          ...
          >
            {tools.map((tool) => (
              ...
              >
                <div style={{ fontSize: 30, lineHeight: 1, display: 'flex' }}>{tool.icon}</div>
                <div
                  style={{
                    fontSize: 13,
                    fontWeight: 600,
