...
import { TransferGenericCTA } from '@/components/affiliate/TransferGenericCTA'
...
export async function generateMetadata({
  ...
  const guide = getGuide('transporte-entre-ciudades')
  if (!guide) return {}

  const path = locale === 'es' ? '/es/viajes/transporte' : '/en/travel/transport'
  return buildPageMetadata({
    ...
    alternates: {
      languages: {
        'es-419': `${SITE_URL}/es/viajes/transporte`,
        en: `${SITE_URL}/en/travel/transport`,
        'x-default': `${SITE_URL}/es/viajes/transporte`,
      },
    },
  })
}
...
export default async function TransportePage({
  ...
  const path =
    locale === 'es' ? `/${lang}/viajes/transporte` : `/${lang}/travel/transport`
  ...
  return (
    <>
      ...
      <article className="mx-auto max-w-4xl space-y-12 py-6">
        ...
        {/* Transfer providers CTA */}
        <TransferGenericCTA lang={locale} />
