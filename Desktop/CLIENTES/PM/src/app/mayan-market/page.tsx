...
export default function MayanMarketPage() {
  const { t, language } = useLanguage();

  useEffect(() => {
    document.title = t('page.mayanMarket.meta.title');
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) {
      metaDesc.setAttribute('content', t('page.mayanMarket.meta.description'));
    }
  }, [language, t]);
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="relative w-full max-w-4xl mx-auto aspect-[12/5] rounded-lg overflow-hidden shadow-md mb-8 group">
              <Image
                ...
                alt={t('page.mayanMarket.heading')}
                fill
                style={{ objectFit: 'cover' }}
                sizes="(max-width: 1200px) 100vw, 80vw"
                className="transition-transform duration-300 group-hover:scale-110"
                data-ai-hint="market crafts"
              />
            </div>
          </div>
        </section>
            ...
            <Card className="shadow-xl rounded-xl my-8 transition-all duration-300 hover:shadow-2xl hover:scale-[1.02]">
              <CardHeader className="text-center">
                <CardTitle className="font-headline font-semibold text-2xl md:text-3xl text-foreground">
                  {t('page.mayanMarket.heading')}
                </CardTitle>
              </CardHeader>
              <CardContent className="text-center py-8 md:py-12">
                <p
                  className="text-lg text-foreground/80 max-w-xl mx-auto mb-8"
                  dangerouslySetInnerHTML={{ __html: t('page.mayanMarket.p1').replace('<span class="font-semibold text-accent">', '<span class="font-semibold text-foreground">') }}
                />
                <p className="text-md text-foreground/70 max-w-xl mx-auto mb-8">
                  {t('page.mayanMarket.p2')}
                </p>
