...
export async function generateMetadata({
  ...
  const guide = getGuide('vuelos-desde-mexico')
  if (!guide) return {}

  const path =
    locale === 'es' ? '/es/viajes/vuelos/desde-mexico' : '/en/travel/flights/from-mexico'
  return buildPageMetadata({
    ...
    alternates: {
      languages: {
        'es-419': `${SITE_URL}/es/viajes/vuelos/desde-mexico`,
        en: `${SITE_URL}/en/travel/flights/from-mexico`,
        'x-default': `${SITE_URL}/es/viajes/vuelos/desde-mexico`,
      },
    },
  })
}

export default async function VuelosDesdeMexicoPage({
  params,
}: {
  params: Promise<{ lang: string }>
}) {
  ...
  const path =
    locale === 'es'
      ? `/${lang}/viajes/vuelos/desde-mexico`
      : `/${lang}/travel/flights/from-mexico`
