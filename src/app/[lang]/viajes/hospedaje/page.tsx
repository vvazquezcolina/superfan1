...
export async function generateMetadata({
  ...
  const guide = getGuide('hospedaje-general')
  if (!guide) return {}

  const path = locale === 'es' ? '/es/viajes/hospedaje' : '/en/travel/accommodation'
  return buildPageMetadata({
    ...
    alternates: {
      languages: {
        'es-419': `${SITE_URL}/es/viajes/hospedaje`,
        en: `${SITE_URL}/en/travel/accommodation`,
        'x-default': `${SITE_URL}/es/viajes/hospedaje`,
      },
    },
  })
}
...
export default async function HospedajePage({
  ...
  const path =
    locale === 'es' ? `/${lang}/viajes/hospedaje` : `/${lang}/travel/accommodation`
  ...
  // Index of the "tipos de alojamiento" section (first section, index 0)
  const TIPOS_SECTION_INDEX = 0
  ...
  return (
    <>
      ...
      <article className="mx-auto max-w-4xl space-y-12 py-6">
        ...
        {/* Featured affiliate CTAs */}
        {guide.affiliateCTAs.map((cta) => (
          <div
            key={cta.partner}
            className="rounded-lg border border-primary/30 bg-primary/5 p-4 my-6 mx-auto max-w-prose"
          >
            <AffiliateLink
              href={cta.url}
              partner={cta.partner}
        ...
        {/* Content sections with BookingWidget after tipos de alojamiento */}
        {guide.sections.map((section, index) => (
          <div key={index}>
            <CitySection
              ...
              id={`seccion-${index + 1}`}
            />
            {index === TIPOS_SECTION_INDEX && (
              <BookingWidget
                cityName={locale === 'es' ? 'Mundial FIFA 2026' : 'FIFA World Cup 2026'}
                citySlug="mundial-2026"
                lang={locale}
                dict={dict.affiliate}
              />
            )}
