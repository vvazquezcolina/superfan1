...
export default function EventosPage() {
  ...
  useEffect(() => {
    document.title = t('page.eventos.meta.title');
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) {
      metaDesc.setAttribute('content', t('page.eventos.meta.description'));
    }
  }, [language, t]);
            ...
            <Card className="shadow-xl rounded-xl my-8 transition-all duration-300 hover:shadow-2xl hover:scale-[1.02]">
              <CardHeader className="text-center">
                <CardTitle className="font-headline font-semibold text-2xl md:text-3xl text-foreground">
                  {t('page.eventos.heading')}
                </CardTitle>
              </CardHeader>
              <CardContent className="text-center py-8 md:py-12">
                <p
                  className="text-lg text-foreground/80 max-w-xl mx-auto mb-8"
                  dangerouslySetInnerHTML={{ __html: t('page.eventos.p1').replace('<span class="font-semibold text-accent">', '<span class="font-semibold text-foreground">') }}
                />
