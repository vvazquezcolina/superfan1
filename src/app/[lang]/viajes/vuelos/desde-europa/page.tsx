...
export async function generateMetadata({
  ...
  const guide = getGuide('vuelos-desde-europa')
  if (!guide) return {}

  const path =
    locale === 'es' ? '/es/viajes/vuelos/desde-europa' : '/en/travel/flights/from-europe'
  return buildPageMetadata({
    ...
    alternates: {
      languages: {
        'es-419': `${SITE_URL}/es/viajes/vuelos/desde-europa`,
        en: `${SITE_URL}/en/travel/flights/from-europe`,
        'x-default': `${SITE_URL}/es/viajes/vuelos/desde-europa`,
      },
    },
  })
}

export default async function VuelosDesdeEuropaPage({
  params,
}: {
  params: Promise<{ lang: string }>
}) {
  ...
  const path =
    locale === 'es'
      ? `/${lang}/viajes/vuelos/desde-europa`
      : `/${lang}/travel/flights/from-europe`
  ...
  const backLabel =
    locale === 'es' ? 'Volver a la guia de vuelos' : 'Back to flight guide'
  const backPath = locale === 'es' ? `/${lang}/viajes/vuelos` : `/${lang}/travel/flights`
