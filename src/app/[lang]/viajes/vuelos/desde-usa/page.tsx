...
export async function generateMetadata({
  ...
  const guide = getGuide('vuelos-desde-usa')
  if (!guide) return {}

  const path =
    locale === 'es' ? '/es/viajes/vuelos/desde-usa' : '/en/travel/flights/from-usa'
  return buildPageMetadata({
    ...
    alternates: {
      languages: {
        'es-419': `${SITE_URL}/es/viajes/vuelos/desde-usa`,
        en: `${SITE_URL}/en/travel/flights/from-usa`,
        'x-default': `${SITE_URL}/es/viajes/vuelos/desde-usa`,
      },
    },
  })
}

export default async function VuelosDesdeUSAPage({
  params,
}: {
  params: Promise<{ lang: string }>
}) {
  ...
  const path =
    locale === 'es'
      ? `/${lang}/viajes/vuelos/desde-usa`
      : `/${lang}/travel/flights/from-usa`
